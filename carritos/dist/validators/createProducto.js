...
const vaidateCreateProducto = (body) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!carrito) {
        return [false, `El id del carrito no existe`];
    }
    ...
    const producto = yield axios_1.default.get(`http://localhost:3000/api/producto/${body.idProducto}`);
    if (!producto.data) {
        return [false, `El id del producto no existe`];
    }
    if (!producto.data.inventario) {
    ...
    if (producto.data.inventario.stock < body.cantidad + cantidadProductosEnElCarrito) {
